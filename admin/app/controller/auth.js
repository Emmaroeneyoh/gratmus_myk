...
const {  appPassword, adminpasswordjwt } = require("../../../general/core/utils");
...
const adminSignupController = async (req, res, next) => {
  ...
  try {
    ...
    const admin = await AdminModel.findOne({ username: adminname });
    if (customer || admin) {
    ...
    const data = {
      userEmail,adminname ,
      Harshpassword,phone , role ,  firstname,
      lastname,
